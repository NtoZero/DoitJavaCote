...
public class S50_1717_SOF implements P50_1717 {
    ...
    // 이렇게 하면 스택 오버 플로우 에러가 발생한다. => 다르게 풀어보자.
    static int union(int a, int b) {
        //인덱스의 요소값은 대표 노드이므로 대표 노드 간 합집합으로 계산해준다.
        int superA = arr[a];
        int superB = arr[b];

        // 인덱스와 밸류가 같은지 확인한다. (대표 노드인지 확인)
        if(superA == a && superB == b) { // a, b 모두 대표 노드일 때
            arr[b] = arr[a]; // 대표 노드끼리 합연산 union 한다.
            return arr[a]; // 대표 노드를 반환한다.
        } else {
            arr[b] = union(superA, superB); //대표 노드간 재귀
        }
        return arr[a];
    }

    static String find(int a, int b) { // a와 b의 대표 노드가 같은지 확인하는 연산

        if(arr[a]==arr[b]) return "YES\n"; // 틀린 로직. 마지막 연산에서 find가 동작해야 옳은 답이 된다.
        else
            return "NO\n";
    }
}
...
/*
    ...
    union(3, 7)
    0 1 2 3 4 5 6 7 8
    0 1 2 1 4 5 7 3 8 (arr[7] -> 3) => arr[3] (1) != 3 이 아니므로 한 번 더 재귀
    0 1 2 1 4 5 7 1 8 (arr[7] -> arr[3]) => arr[3] (1) == 1 이므로 재귀 끝

    union(4, 2)
    0 1 2 3 4 5 6 7 8
    0 1 4 1 4 5 7 1 8 (arr[2] -> arr[4])

    union(1, 1) (a, b가 같은 경우)
    0 1 2 3 4 5 6 7 8
    0 1 4 1 4 5 7 1 8 (arr[1] -> arr[1])
